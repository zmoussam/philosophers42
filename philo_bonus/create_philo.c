...
/*   Updated: 2022/09/15 22:45:19 by zmoussam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_philo(t_arg args_info, t_philos *philosopher, sem_t *forks)
{
	...
	long long time;
	i = 0;
	forks = sem_open(SEM_NAME_1, O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, args_info.n_of_p / 2);
	while(i < args_info.n_of_p)
	{
		...
		if(philosopher[i].fork_id == 0)
		{
			...
			philosopher[i].time_of_last_meal = philosopher[i].time;
